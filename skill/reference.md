...
- **Syntax highlighting**: `"theme": "default"` colors keywords, strings, numbers and comments (names and
  ...
  without losing token colors, tween a span's `background` only. To pulse every token of one kind, iterate
  `python_tokens(src)` and add a `chars` span per token (`examples/syntax_theme.py`).
