...
part it talks about with spans whose `background` tweens from `transparent` to a highlight and back, so the
highlight fades while the code underneath, colored by the built-in syntax theme, stays put.
"""
...
BG, INK, MUTED, HI_BG = "#0e0e11", "#e8e8ee", "#8a8a94", "#3a3a52"
SIZE = 0.34
REVEAL_AT, REVEAL_EACH, REVEAL_DUR = 0.6, 0.55, 0.45
HOLD, RAMP = 2.8, 0.3

code = Code("fn", content=SRC, size=SIZE, fill=INK, theme="default")
...
for k, (text, selections) in enumerate(STEPS):
    ...
    for j, sel in enumerate(selections):
        code.select(f"step{k}_{j}", **sel, background=keyframes(window(t0, t1, "transparent", HI_BG), ease="out_quad"))
